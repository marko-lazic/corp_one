    ...
    fn build(&self, app: &mut AppBuilder) {
        app.add_system_set(
            SystemSet::on_enter(GameState::Loading)
                .with_system(Self::print_loading.system())
                .with_system(Self::start_loading.system()),
        )
        .add_system_set(
            SystemSet::on_update(GameState::Loading).with_system(Self::check_state.system()),
        )
        .add_system_set(
            SystemSet::on_exit(GameState::Loading).with_system(Self::clean_up_loading.system()),
        );
    }
}
