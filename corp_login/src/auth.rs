...
use sqlx::SqlitePool;
use anyhow::Result;
use corp_types::{User, CreateUserRequest, LoginRequest, Token};
...
pub async fn authenticate_user_and_generate_token(pool: &SqlitePool, request: LoginRequest) -> Result<Option<(User, Token)>> {
    let user = sqlx::query_as::<_, User>(
